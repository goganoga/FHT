...
namespace FHT{
    Hendler::Hendler(){}
    Hendler::~Hendler(){}
    void Hendler::addUniqueHendler(std::string id, std::function<void(iHendler::data)> func){
    ...
    std::function<void(iHendler::data)> Hendler::getUniqueHendler(std::string id){
        if (auto a = mapHendler.find(id); a != end(mapHendler))
            return a->second;
        return nullptr;
