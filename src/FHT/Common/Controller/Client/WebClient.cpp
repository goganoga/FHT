...
namespace FHT{
    ...
    webClient::hostnameValidationResult webClient::matchesSubjectAlternativeName(const char* hostname, const X509* serverCert) {
    ...
    webClient::hostnameValidationResult webClient::matchesCommonName(const char* hostname, const X509* serverCert) {
    ...
    webClient::hostnameValidationResult webClient::validate_hostname(const char* hostname, const X509* serverCert) {
...
#endif
    int webClient::cert_verify_callback(X509_STORE_CTX* x509Ctx, void* arg) {
			char cert_str[256];
			...
			if (ok_so_far) {
                res = validate_hostname(host, serverCert);
				switch (res) {
				case MatchFound:
					res_str = "MatchFound";
					break;
