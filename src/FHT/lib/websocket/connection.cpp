...
void response_write_cb(struct bufferevent *bev, void *ctx) {
	...
	if (conn && conn->bev) {
		...
		// LOG >> std::cout <<  conn->ws_req_str.c_str() << std::endl;
		// LOG >> std::cout <<  conn->ws_resp_str.c_str() << std::endl;
...
int32_t send_a_frame(ws_conn_t *conn, const frame_buffer_t *fb) {
	try {
		if (conn)
			return bufferevent_write(conn->bev, fb->data, fb->len) ? 500 : 200;
		throw;
	}
	catch (const std::exception& e) {
		std::cerr << "Error write buffer: " << e.what() << std::endl;
		return 404;
	}
}
...
void frame_read_cb(struct bufferevent *bev, void *ctx) {
	...
	case ONE:
		{
			// LOG >> std::cout <<  "---- STEP 1 ----" << std::endl;
			std::unique_ptr<char> tmp(new char[conn->ntoread]);
			bufferevent_read(bev, tmp.get(), conn->ntoread);
			//parse header
			if (parse_frame_header(tmp.get(), conn->frame) == 0) {
				// LOG >> std::cout <<  "FIN         = " << conn->frame->fin << std::endl;
				// LOG >> std::cout <<  "OPCODE      = " << conn->frame->opcode << std::endl;
				// LOG >> std::cout <<  "MASK        = " << conn->frame->mask << std::endl;
				// LOG >> std::cout <<  "PAYLOAD_LEN = " << conn->frame->payload_len << std::endl;
	...
	case TWO:
		{
			// LOG >> std::cout <<  "---- STEP 2 ----" << std::endl;
			std::unique_ptr<char> tmp(new char[conn->ntoread]);
			bufferevent_read(bev, tmp.get(), conn->ntoread);
			if (conn->frame->payload_len == 126) {
				conn->frame->payload_len = ntohs(*(uint16_t*)tmp.get());
				// LOG >> std::cout <<  "PAYLOAD_LEN = "<< conn->frame->payload_len << std::endl;
			} else if (conn->frame->payload_len == 127) {
				conn->frame->payload_len = myntohll(*(uint64_t*)tmp.get());
				// LOG >> std::cout <<  "PAYLOAD_LEN = "<< conn->frame->payload_len << std::endl;
			}
			conn->step = THREE;
			conn->ntoread = 4;
	...
	case THREE:
		{
			// LOG >> std::cout <<  "---- STEP 3 ----" << std::endl;
			std::unique_ptr<char> tmp(new char[conn->ntoread]);
			bufferevent_read(bev, tmp.get(), conn->ntoread);
			memcpy(conn->frame->masking_key, tmp.get(), conn->ntoread);
			if (conn->frame->payload_len > 0) {
				...
				if (conn->frame->fin == 1 && conn->frame->opcode == 0x8) {
					...
					// LOG >> std::cout <<  "send a close frame" << std::endl;
	...
	default:
		// LOG >> std::cout <<  "---- STEP UNKNOWN ----" << std::endl;
		// LOG >> std::cout <<  "exit" << std::endl;
		exit(-1);
		break;
	}
