...
void print_frame_info(const frame_buffer_t *fb) {
	...
	if (payload_len == 0) {
		...
		// LOG >> std::cout <<  "126" <<std::endl;
		payload_len = myntohs(*(uint16_t*)(p + 2));
		tmp = new char[payload_len + 1];
		tmp[payload_len] = 0;
		memcpy(tmp, p + 2 + 2, payload_len);
	} else if (payload_len == 127) {
		// LOG >> std::cout <<  "127" << std::endl;
		payload_len = myntohll(*(uint64_t*)(p + 2));
		tmp = new char[payload_len + 1];
		tmp[payload_len] = 0;
		memcpy(tmp, p + 2 + 8, payload_len);
	}
	// LOG >> std::cout << "fin = " << fin << std::endl;
	// LOG >> std::cout << "opcode = " << opcode << std::endl;
	// LOG >> std::cout << "mask = " << mask << std::endl;
	// LOG >> std::cout << "payload_len = " << payload_len << std::endl;
	if (tmp) {
		// LOG >> std::cout <<  "payload = "<< tmp << std::endl;
		delete[] tmp;
	} else {
		// LOG >> std::cout <<  "payload = NULL" << std::endl;
	}
	// LOG >> std::cout <<  "--------------------" << std::endl;
}
